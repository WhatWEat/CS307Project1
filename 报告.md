...
针对每个表内数据的导入，利用线程池开设12个线程，将每个表切分成12份，并行导入。考虑到如果每个线程中表中数据量过小使得并行节省的时间反而小于线程调度需要的时间。
为此，我设置切分块的大小为$$ size = max(2000, \frac{tablesize}{12})$$ 

##### 方法五： 双级多线程

除了表内并行的插入数据外，每个表之间也并行插入数据，由于表间可能存在**外键约束**导致某些表的导入必须依赖一定的顺序无法彻底并行，为此
